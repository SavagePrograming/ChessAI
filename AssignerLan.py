...
class Assigner():
    ...
    def start(self):
        print 'Name of Game:'
        self.game = raw_input()
        Functions.RunFile("Recieverdominant.py")
        true = True
        while true:
            # try:
            self.mainloop()
    ...
    def mainloop(self):
        ...
        for message in messages:
            ...
            if message[0] == 'Calc':
                ...
                self.str_color = message[2]
                self.level = int(message[3])
                self.new_moves = Functions.generate_moves(self.board, self.str_color)
                self.finished_moves = []
                self.working_moves = []
                ...
                for move in self.working_moves:
                    if move.tarsq.str_Name == move_.tarsq.str_Name and move.locsq.str_Name == move_.locsq.str_Name:
        ...
        if not self.working_moves and not self.new_moves and self.working:
            ...
            if not self.finished_moves:
                self.win = 'stalemate'
                self.send_exit()
                raise Player.BreakError('Stalemate')
            ##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            #clears king
            ##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            self.king = 0
            ...
            ##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            for i in self.board.array_square_Squares:
                if not i.str_color == self.str_color:
                    if i.int_Rule_Num == 6:
                        self.yking = i
            ##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    def receive(self):
        msgs, test = Recieve.receive('A', self.used)
        if test:
    ...
    def send_processors(self):
        for move in self.new_moves:
            for processor in self.processors:
                if processor[1]:
                    Send.send_msg(processor[0], 'PR-Measure-' + move.toString(self.board) + '-' +
    ...
    def send_exit(self):
        for processor in self.processors:
            Send.send_msg(processor[0], 'PR-Exit-' + self.win)
        Send.send_msg(self.player, ('PL-Exit-' + self.win))

    def send_move(self):
        Send.send_msg(self.player, 'PL-Done-' + self.best_move.toString(self.board))

...
class Square(pygame.sprite.Sprite):
    ...
    def check(self, board):
        ...
        #==================================================================================
            ...
            if self.color == 'w':
                test = True
                sq = board.b[(self.x + 1)][ self.y]
                if sq.image == 0:
                    self.p_m.append([self.x + 1, self.y])
                else:
                    test = False

                sq = board.b[(self.x) + 1][(self.y) + 1]
                if not sq.image == 0 and not sq.color == self.color:
                    self.p_m.append([self.x + 1, self.y + 1])
                sq = board.b[self.x + 1][self.y - 1]
                if not sq.image == 0 and not sq.color == self.color:
                    self.p_m.append([self.x + 1, self.y - 1])

                if self.x == 1:
                    sq = board.b[self.x + 2][self.y]
                    if not sq.image == 0:
                        test = False
                else:
                    test = False
                ...
                if test:
                    self.p_m.append([self.x + 2, self.y])
                if self.x == 7:
            ...
            if self.color == 'b':
                test = True
                for sq in board.sq:
                    if sq.location == [self.x - 1, self.y]:
                        if sq.image == 0:
                            self.p_m.append([self.x - 1, self.y])
                        else:
                            test = False
                    elif sq.location == [self.x - 1, self.y + 1]:
                        if not sq.image == 0 and not sq.color == self.color:
                            self.p_m.append([self.x - 1, self.y + 1])
                    elif sq.location == [self.x - 1, self.y - 1]:
                        if not sq.image == 0 and not sq.color == self.color:
                        ...
                        if sq.location == [self.x - 2, self.y]:
                            if not sq.image == 0:
                                test = False
                    elif not self.x == 6:
                        test = False
            ...
            for number in range(0, 8):
                ...
                for square in board.sq:
                    if square.location[0] == self.x:
                        if square.location == [self.x, number * 1]:
                            SQ = square
                        if self.y > number * 1:
                            if self.y > square.y > number * 1:
                ...
                if test:
                    if SQ.image == 0:
                        self.p_m.append([self.x, number * 1])
                    elif not SQ.color == self.color:
                        self.p_m.append([self.x, number * 1])
                test = True
                for square in board.sq:
                    if square.location[1] == self.y:
                        if square.location == [number * 1, self.y]:
                            SQ = square
                        if self.x > number * 1:
                            if self.x > square.x > number * 1:
                                if not square.image == 0:
                                    test = False
                        if self.x < number * 1:
                            if self.x < square.x < number * 1:
                ...
                if test:
                    if SQ.image == 0:
                        self.p_m.append([number * 1, self.y])
                    elif not SQ.color == self.color:
                        self.p_m.append([number * 1, self.y])
                        #==================================================================================
                        #For Knight
            ...
            for j in [1, -1]:
                for h in [1, -1]:
                    for number in range(-8, 8):
                        for square in board.sq:
                            if square.location == [self.x + j * number * 1, self.y + h * number * 1]:
                                List.append(square)
            ...
            for i in [1, -1, 0, 0]:
                for j in [1, -1, 0, 0]:
                    if not j == 0 or not i == 0:
                        for sq in board.sq:
                            if sq.location == [self.x + i * 1, self.y + j * 1]:
                                if sq.image == 0:
